...
interface EnvironmentConfig {
  ...
  conditionalRequirements?: Record<string, { 
    condition: () => boolean;
    variables: string[];
    description: string;
  }>;
...
export class EnvironmentValidator {
  ...
  validate(): ValidationResult {
    ...
    for (const [variable, description] of Object.entries(this.config.required)) {
    ...
    for (const [variable, description] of Object.entries(this.config.optional)) {
      if (!process.env[variable]) {
    ...
    if (this.config.conditionalRequirements) {
      for (const [name, requirement] of Object.entries(this.config.conditionalRequirements)) {
  ...
  private addSuggestions(result: ValidationResult): void {
    const missingVariables = result.errors.map(error => 
      error.match(/Missing.*environment variable: (\w+)/)?.[1]
    ).filter(Boolean);

    if (missingVariables.includes("NEXT_PUBLIC_CONVEX_URL")) {
      result.suggestions.push(
        "Set up Convex deployment at https://convex.dev"
      );
    }
  ...
  getStatusReport(): {
    ...
    const configuredVariables = Object.keys(process.env).filter(key => 
...
const keeperAIEnvironmentConfig: EnvironmentConfig = {
  required: {
    NEXT_PUBLIC_CONVEX_URL: "Convex database connection URL for backend services"
  },
  optional: {
    NEXT_PUBLIC_GOOGLE_BOOKS_API_KEY: "Google Books API access for enhanced search results and book metadata",
    NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY: "Clerk authentication for user accounts and favorites (future phases)",
    UPSTASH_REDIS_REST_URL: "Redis caching service URL for improved performance",
    UPSTASH_REDIS_REST_TOKEN: "Redis authentication token",
    NEXT_PUBLIC_VERCEL_ANALYTICS_ID: "Vercel analytics for usage tracking and optimization"
  },
  conditionalRequirements: {
    ...
    "Production Environment": {
      ...
      description: "Google Books API key is recommended for production to avoid rate limiting"
    }
  }
};
 ...
 */
export const environmentValidator = new EnvironmentValidator(keeperAIEnvironmentConfig);
